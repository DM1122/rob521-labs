...
class wheelRadiusEstimator:
    def __init__(self):
        ...
        rospy.Subscriber(
            "sensor_state", SensorState, self.sensorCallback
        )  # Subscribe to the sensor state msg
    ...
    def sensorCallback(self, msg):
        ...
        if self.left_encoder_prev is None or self.left_encoder_prev is None:
            ...
            self.del_left_encoder += self.safeDelPhi(
                self.left_encoder_prev, msg.left_encoder
            )
            self.del_right_encoder += self.safeDelPhi(
    ...
    def startStopCallback(self, msg):
        input_velocity_mag = np.linalg.norm(
        ...
        if self.isMoving is False and np.absolute(input_velocity_mag) > 0:
            ...
            radius = (
                2 * DRIVEN_DISTANCE / (self.del_left_encoder + self.del_right_encoder)
            )
