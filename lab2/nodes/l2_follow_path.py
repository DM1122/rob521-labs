...
from scipy.spatial.distance import (
    cityblock,
)  #  calcuate the manhattan distance between two (x, y, z) vectors
import rospy
import tf2_ros
...
class PathFollower:
    def __init__(self):
        ...
        self.collision_radius_pix = (
            COLLISION_RADIUS / self.map_resolution
        )  # map_resolution m/pix
        ...
        self.all_opts = np.array(np.meshgrid(TRANS_VEL_OPTS, ROT_VEL_OPTS)).T.reshape(
            -1, 2
        )  # (44, 2) = (# of possible option * (trans_vel, rot_vel))
    ...
    def follow_path(self):
        while not rospy.is_shutdown():
            ...
            local_paths = np.zeros(
                [self.horizon_timesteps + 1, self.num_opts, 3]
            )  # (# of time steps needed to cover the CONTROLHORIZON, options of (v,w), each local path represented by 3 value )
            ...
            for t in range(1, self.horizon_timesteps + 1):
                ...
                for opt in range(self.num_opts):
                    ...
                    # Solve ODE
                    solution = odeint(
                        func=system_dynamics,
                        y0=[x, y, theta],
                        t=t,
                        args=(trans_vel, rot_vel),
                    )
            ...
            for opt in valid_opts:
                ...
                final_cost[opt] = np.linalg.norm(
                    local_paths[-1, opt, :2] - self.cur_goal[:2]
                )
            ...
            if final_cost.size == 0:  # hardcoded recovery if all options have collision
                ...
                best_control = self.all_opts[
                    best_traj_idx
                ]  # pick the bast one in map frame
                self.local_path_pub.publish(
                    utils.se2_pose_list_to_path(local_paths[:, best_traj_idx], "map")
                )
